...
def train(model_path):
    ...
    if hp.train.restore:
        ...
        latest_epoch = sorted(os.listdir(hp.train.checkpoint_dir))[-1]
        print("Latest epoch : ", latest_epoch)
        epoch_start = int(latest_epoch.split("_")[2])
        print("Start epoch : ", epoch_start)
        latest_model_path = os.path.join(hp.train.checkpoint_dir, latest_epoch)
        ...
        print("No previous state, start learning from scratch ...\n")
    ...
    iteration = 0
    for e in range(epoch_start, hp.train.epochs):
        total_loss = 0
    ...
    torch.save(embedder_net.state_dict(), model_path)
